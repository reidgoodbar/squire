...
func startHotCacheServer(ctx context.Context, k *Engine, storeRoot string) (*hotCacheServer, error) {
...
func (s *hotCacheServer) serve(ctx context.Context, k *Engine) {
	for {
...
func handleHotCacheConn(ctx context.Context, k *Engine, conn net.Conn) {
	defer conn.Close()
	for {
		select {
		case <-ctx.Done():
...
func (k *Engine) tryDaemonReplay(ctx context.Context, inv CommandInvocation, family OperatorFamily, phases *PhaseTimings) (RunResult, bool) {
...
func (k *Engine) hotCacheRoundTrip(ctx context.Context, path string, req hotCacheRequest) (hotCacheResponse, error) {
	k.hotCacheMu.Lock()
...
func (k *Engine) hotCacheRoundTripLocked(ctx context.Context, path string, req hotCacheRequest) (hotCacheResponse, error) {
...
func (k *Engine) closeHotCacheClientLocked() {
...
func (k *Engine) shouldSkipHotCache(path, missKey string) bool {
	now := hotCacheNow()
	k.hotCacheMu.Lock()
	defer k.hotCacheMu.Unlock()
...
func (k *Engine) recordHotCacheUnavailable(path string) {
	k.hotCacheMu.Lock()
	defer k.hotCacheMu.Unlock()
...
func (k *Engine) recordHotCacheMiss(missKey string) {
	k.hotCacheMu.Lock()
	defer k.hotCacheMu.Unlock()
	if k.hotCacheMisses == nil {
...
func (k *Engine) recordHotCacheHit(path, missKey string) {
	k.hotCacheMu.Lock()
	defer k.hotCacheMu.Unlock()
...
func hotCacheSocketDir() string {
	if dir := preferredEnv("SQUIRE_SOCKET_DIR", "SQUIRE_KERNEL_SOCKET_DIR"); dir != "" {
