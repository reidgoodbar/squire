...
func TestMain(m *testing.M) {
	_ = os.Setenv("SQUIRE_SYNC_LEDGER_WRITES", "1")
	os.Exit(m.Run())
}
...
func TestGitDashCNormalizesEffectiveWorkingDirectory(t *testing.T) {
	...
	repo := testRepo(t, ctx)
	parent := t.TempDir()
	k := New(filepath.Join(parent, ".squire", "state"))
...
func TestBackgroundMaintainerProcessLifecycle(t *testing.T) {
	...
	text, err := RuntimeStatus(ctx, repo, storeRoot)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDaemonHotCacheReplaysAcrossEngineInstances(t *testing.T) {
	ctx := context.Background()
	repo := testRepo(t, ctx)
	...
	if replay.Mode != ModeReplay {
		...
		t.Fatalf("daemon hot cache did not replay across proof-cache instances; last mode=%s diagnostics=%v socket=%s probe=%+v probeErr=%v", replay.Mode, replay.Diagnostics, hotCacheSocketPath(storeRoot), probe, probeErr)
	}
...
func TestRuntimeStatusShowsPreparedWorld(t *testing.T) {
	ctx := context.Background()
	repo := testRepo(t, ctx)
	storeRoot := DefaultStoreRoot(repo)
	...
	status, err := RuntimeStatus(ctx, repo, storeRoot)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNonGitWorkspaceWarmReplaysFileInspectionAcrossEngineInstances(t *testing.T) {
	ctx := context.Background()
	workspace := t.TempDir()
...
func TestSetupAndStatusSurfaces(t *testing.T) {
	...
	status, err := RuntimeStatus(ctx, repo, DefaultStoreRoot(repo))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRuntimeStatusSummaryIsCompact(t *testing.T) {
	ctx := context.Background()
	repo := testRepo(t, ctx)
	storeRoot := DefaultStoreRoot(repo)
	if _, err := Warm(ctx, repo, storeRoot); err != nil {
	...
	status, err := RuntimeStatusSummary(ctx, repo, storeRoot)
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{"Squire status", "readiness:", "repo_oracle:", "repo_root:", "native_fallback: available", "runtime_decisions: replay_or_native", "enabled_fast_paths:", "prepared_entries:", "hot_snapshot:", "background_maintainer:"} {
		if !strings.Contains(status, want) {
			t.Fatalf("runtime status summary missing %q:\n%s", want, status)
		}
	}
	for _, notWant := range []string{"current_repo_oracle_state", "invalidation_epoch:", "process_guard:"} {
		if strings.Contains(status, notWant) {
			t.Fatalf("runtime status summary should not include detailed section %q:\n%s", notWant, status)
		}
	}
}

func TestRuntimeStatusShowsLatestBenchmark(t *testing.T) {
	ctx := context.Background()
	repo := testRepo(t, ctx)
