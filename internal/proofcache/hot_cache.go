...
func (k *Engine) tryPreparedReplay(inv CommandInvocation, family OperatorFamily, phases *PhaseTimings) (RunResult, bool) {
...
func (k *Engine) recordPreparedReplay(candidate preparedReplay) {
...
func (k *Engine) tryDiskPreparedReplay(inv CommandInvocation, family OperatorFamily, phases *PhaseTimings) (RunResult, bool) {
	if preferredEnv("SQUIRE_ENABLE_FOREGROUND_DISK_REPLAY", "SQUIRE_KERNEL_ENABLE_FOREGROUND_DISK_REPLAY") != "1" {
		return RunResult{}, false
	}
...
func (k *Engine) ensurePreparedReplayCacheLoaded(phases *PhaseTimings) {
	if k.Store == nil {
		return
	}
...
func (k *Engine) findPreparedReplay(inv CommandInvocation, phases *PhaseTimings) (preparedReplay, []string, bool) {
	if !isHotPreparedReplayCandidate(inv.PolicyArgv) {
...
func (k *Engine) residentPreparedReplayCandidates(command string, phases *PhaseTimings) ([]preparedReplay, []string) {
	lockStart := time.Now()
	k.mu.Lock()
	phases.LockWaitMS += elapsedMS(lockStart)
