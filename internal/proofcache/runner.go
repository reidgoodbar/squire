...
func (k *Engine) Run(ctx context.Context, sessionID, cwd string, argv []string) RunResult {
	...
	if !requiresProofWorld(inv.PolicyArgv) {
...
func (k *Engine) WaitForegroundObservations() {
	if k == nil {
		return
	}
...
func requiresProofWorld(argv []string) bool {
...
func (k *Engine) nativeFallback(ctx context.Context, op Operation, key string, ledger *ValidityLedger, diagnostics []string, proof *ProofRecord, phases PhaseTimings) RunResult {
...
func (k *Engine) loadLedger(phases *PhaseTimings) (*ValidityLedger, bool, []string) {
	lockStart := time.Now()
	k.mu.Lock()
...
func (k *Engine) materializeOutput(entry *LedgerEntry) ([]byte, []byte, error) {
	k.mu.Lock()
...
func (k *Engine) precomputeFastPathOutputs(ctx context.Context, cwd, sessionID string, observedArgv []string, observed NativeResult, ws WorldState, ledger *ValidityLedger, phases *PhaseTimings) error {
...
func (k *Engine) storeReplayableObservation(argv []string, sessionID, cwd string, observed NativeResult, ws WorldState, ledger *ValidityLedger, phases *PhaseTimings, decision Mode) error {
...
func (k *Engine) saveLedgerSync(ledger *ValidityLedger, phases *PhaseTimings) error {
	if k.Store == nil || ledger == nil {
		return nil
	}
	k.mu.Lock()
...
func (k *Engine) scheduleLedgerSave() {
	if k.Store == nil {
		return
	}
	if preferredEnv("SQUIRE_SYNC_LEDGER_WRITES", "SQUIRE_KERNEL_SYNC_LEDGER_WRITES") == "1" {
		k.flushLedgerNow()
		return
	}
	k.mu.Lock()
...
func (k *Engine) flushScheduledLedgerSave() {
	if k.Store == nil {
		return
	}
	k.mu.Lock()
