...
func (k *Engine) tryHotSnapshotReplay(inv CommandInvocation, family OperatorFamily, phases *PhaseTimings) (RunResult, bool) {
	if k == nil || k.Store == nil {
...
func (k *Engine) PreloadHotSnapshot() bool {
	if k == nil || k.Store == nil {
		return false
	}
	path := hotCacheSnapshotPath(k.Store.Root)
...
func (k *Engine) readHotSnapshotResponse(path string, inv CommandInvocation) (hotCacheResponse, bool) {
	if path == "" {
