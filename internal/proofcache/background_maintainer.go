...
func defaultBackgroundMaintainerCommand(cwd, storeRoot string, opts BackgroundMaintainerOptions, log *os.File) (*exec.Cmd, error) {
	...
	args := []string{"runtime", "maintain", "--duration", opts.Duration.String(), "--poll-interval", opts.PollInterval.String()}
	cmd := exec.Command(exe, args...)
	cmd.Dir = cwd
	cmd.Stdout = log
	cmd.Stderr = log
	cmd.Env = os.Environ()
	cmd.Env = append(cmd.Env, "SQUIRE_STORE_ROOT="+storeRoot, "GIT_OPTIONAL_LOCKS=0")
	return cmd, nil
}
