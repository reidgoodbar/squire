...
func hotStoreRoot(gitDirAbs string) string {
	return filepath.Join(gitDirAbs, "squire", "state")
}
...
func (k *Engine) FastReplay(ctx context.Context, sessionID, cwd string, argv []string) (*RunResult, bool) {
	inv := NormalizeInvocation(cwd, argv)
	return k.FastReplayInvocation(ctx, sessionID, inv)
}

func (k *Engine) FastReplayInvocation(ctx context.Context, sessionID string, inv CommandInvocation) (*RunResult, bool) {
	_ = sessionID
	select {
	case <-ctx.Done():
		return nil, false
